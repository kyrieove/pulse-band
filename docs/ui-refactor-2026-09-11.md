...
1. **MiniBar 圆环数值与详情卡对不上**（圆环 Codex 64%，同一时刻详情卡 5 小时剩余 100%）。
   两处读的是同一个表达式 `toRemainingPercent(quotas[key]?.pct5h)`，静态读代码排除了
   key 别名、`mountedAgent` 滞后、`pct5hOverride` 三条路径。
   当前假设是 `quota-collector.ts` 里 live 接口与本地 `rollout-*.jsonl` 兜底两条数据源，
   在 `expire()` 把过期窗口的 `pct` 归零之后产生了版本分歧，经由 IPC 先后推达。
   **未闭环**，需要在 Codex 分支打时间戳日志抓现场。
