...
```text
origin    https://github.com/wtz35/pulse-band.git
upstream  https://github.com/kyrieove/pulse-band.git
```
...
发生冲突时逐个解决并测试，然后执行 `git rebase --continue`。若只是想退出本次变基，可执行 `git rebase --abort`，不会丢失变基前的分支状态。`--force-with-lease` 仅用于变基后的个人功能分支；不要对共享的 `main` 使用强制推送。
