...
- **Pulse 与小米运动健康的双主机业务共存尚未证实**：当前测试路径不支持它（Pulse 在小米运动健康
  连接时建立连接失败），但也不成立"硬件/协议上根本不可能"的结论。实测中成败只与手环是否处于
  「连接新手机」模式相关，与手机是否连着无关。详见
  `../docs/protocol/` 与本地审计记录；未排除的混淆是手环关掉手机蓝牙后可能仍残留已死链路
  （蓝牙监督超时），干净的控制是重启手环（保留绑定）后再测。
